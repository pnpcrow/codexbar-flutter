...
/// ClawRouter API fetch strategy.
/// Uses CLAWROUTER_API_KEY to fetch usage data.
class ClawRouterAPIFetchStrategy extends FetchStrategy {
  @override
  String get id => 'clawrouter.api';
  ...
  Future<bool> isAvailable(ProviderFetchContext context) async {
    ...
    return resolver.resolve(UsageProvider.clawrouter, env: context.env) != null;
  }

  @override
  Future<ProviderFetchResult> fetch(ProviderFetchContext context) async {
    final resolver = CLITokenResolver();
    final resolution = resolver.resolve(UsageProvider.clawrouter, env: context.env);
    if (resolution == null) {
      throw Exception('No ClawRouter API key found');
    }
    ...
    final response = await http.get(
      Uri.parse('https://api.clawrouter.com/v1/usage'),
      headers: headers,
    );

    if (response.statusCode == 401) {
      throw Exception('Invalid ClawRouter API key');
    }

    if (response.statusCode != 200) {
      throw Exception('ClawRouter API error: ${response.statusCode}');
    }
  ...
  UsageSnapshot _parseUsageResponse(Map<String, dynamic> json) {
    ...
    return UsageSnapshot(
      ...
      identity: const ProviderIdentitySnapshot(
        providerID: UsageProvider.clawrouter,
        loginMethod: 'api-key',
      ),
    );
  }
